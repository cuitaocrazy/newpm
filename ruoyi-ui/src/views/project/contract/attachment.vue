<template>
  <div class="app-container contract-attachment">
    <!-- 第一部分：合同信息（只读） -->
    <el-card class="contract-info-card" shadow="never">
      ...
      <el-descriptions :column="2" border v-loading="contractLoading">
...
const { sys_wdlx } = proxy.useDict('sys_wdlx')
...
const handleUpload = () => {
  uploadFormRef.value.validate(valid => {
    if (valid) {
      ...
      formData.append('businessId', contractId.value)
...
const handleShowLog = () => {
  ...
  listAttachmentLog('contract', contractId.value).then(response => {
...
// 初始化
onMounted(() => {
  contractId.value = route.params.contractId
  if (contractId.value) {
    loadContractInfo()
    loadAttachmentList()
  } else {
