<template>
  <div class="app-container">
    <h2 style="margin: 0 0 12px 0; font-weight: bold;">编辑非批次版本</h2>
...
<script setup name="VersionOutManualEdit">
import { ref, computed, onMounted, getCurrentInstance } from 'vue'
import { useRouter, useRoute } from 'vue-router'
import {
  getVersionOutManual, updateVersionOutManual, generateOutLibVersion, getBatchByYear,
  getSysNameByProduct, getOutVersionOptions
...
// 编辑：仅关键字段变更才用 addFlag=2 预览重算，否则沿用原号
...
function submitForm() {
  formRef.value.validate(valid => {
    if (!valid) return
    updateVersionOutManual(form.value).then(() => {
...
onMounted(async () => {
  ...
  loading.value = true
  try {
    const res = await getVersionOutManual(id)
    const d = res.data || {}
    form.value = d
