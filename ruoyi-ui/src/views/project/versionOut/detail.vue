<template>
  <div class="app-container" v-loading="loading">
    ...
    <!-- 二、关联任务 -->
    <el-card shadow="hover" style="margin-bottom: 15px;">
      <template #header><span style="font-size: 16px; font-weight: bold;">二、关联任务</span></template>
      <el-table :data="form.taskList || []" border size="small">
        <el-table-column label="软件中心任务号" prop="taskNo" min-width="160">
          <template #default="{ row }">
            <router-link v-if="row.taskId" class="task-link" :to="`/task/subproject/detail/${row.taskId}`">{{ row.taskNo }}</router-link>
            <router-link v-else-if="row.legacyTaskId" class="task-link" :to="`/project/versionOut/taskSnapshot?legacyId=${row.legacyTaskId}`">{{ row.taskNo }}</router-link>
            <span v-else>{{ row.taskNo }}</span>
          </template>
        </el-table-column>
    ...
    <el-card shadow="hover" style="margin-bottom: 15px;">
      ...
      <el-descriptions :column="3" border>
        ...
        <el-descriptions-item label="最后修改时间" :span="3">{{ form.updateTime }}</el-descriptions-item>
      </el-descriptions>
    </el-card>
