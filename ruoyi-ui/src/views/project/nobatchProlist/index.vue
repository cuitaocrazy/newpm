  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="96px">
      ...
      <el-form-item label="软件中心任务号" prop="taskNo">
        <el-input v-model="queryParams.taskNo" placeholder="任务号" clearable style="width:140px" @keyup.enter="handleQuery" />
      </el-form-item>
      ...
      <template v-if="moreOpen">
        ...
        <el-form-item label="排期状态" prop="scheduleStatus">
          <dict-select v-model="queryParams.scheduleStatus" dict-type="sys_pqzt" placeholder="全部" clearable style="width:140px" />
        </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['project:nobatchProlist:edit']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['project:nobatchProlist:edit']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="dataList" border>
      ...
      <el-table-column label="软件中心任务号" prop="taskNo" min-width="140" show-overflow-tooltip />
...
const data = reactive({
  queryParams: {
    ...
    taskNo: undefined, taskName: undefined, problemNo: undefined,
    problemLevel: undefined, currentStatus: undefined, scheduleStatus: undefined, deptId: undefined,
    whetherDefect: undefined, whetherOvertime: undefined, solutionTimeOverOneDay: undefined, creatorName: undefined,
...
function getList() {
  loading.value = true
  listNobatchProlist(queryParams.value).then(res => {
...
function handleAdd() { router.push('/project/nobatchProlist/add') }
function handleEdit(row) { router.push({ path: '/project/nobatchProlist/edit', query: { problemId: row.problemId } }) }
function handleDetail(row) { router.push({ path: '/project/nobatchProlist/detail', query: { problemId: row.problemId } }) }

function handleDelete(row) {
  proxy.$modal.confirm(`确认删除问题单编号「${row.problemNo}」的记录？`).then(() => {
    return delNobatchProlist(row.problemId)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('/project/nobatchProlist/export', { ...queryParams.value }, `非批次问题单及缺陷_${new Date().getTime()}.xlsx`)
}
