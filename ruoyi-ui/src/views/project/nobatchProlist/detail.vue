<template>
  <div class="app-container">
    <div style="margin-bottom: 12px;">
      ...
      <span style="font-size: 18px; font-weight: bold; margin-left: 12px;">非批次问题单及缺陷详情</span>
    </div>
    ...
    <el-card shadow="hover" style="margin-bottom: 15px;">
      <template #header>
        ...
        <el-button v-hasPermi="['project:nobatchProlist:file']" type="primary" size="small" icon="Upload" style="float:right" @click="openUpload">上传附件</el-button>
      </template>
      <el-table :data="attachmentList" border v-loading="attachmentLoading">
        ...
        <el-table-column label="操作" width="140" align="center">
          <template #default="{ row }">
            <el-button type="primary" link icon="Download" @click="handleDownload(row)" v-hasPermi="['project:nobatchProlist:file','project:nobatchProlist:query']">下载</el-button>
            <el-button type="danger" link icon="Delete" @click="handleDeleteFile(row)" v-hasPermi="['project:nobatchProlist:file']">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
...
<script setup name="NobatchProlistDetail">
import { ref, getCurrentInstance, onMounted } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { getNobatchProlist } from '@/api/project/nobatchProlist'
...
function loadDetail() {
  getNobatchProlist(problemId).then(res => { form.value = res.data || {} })
}
