...
ws["A1"] = "任务号重叠（7个，需确认是否同一任务；新旧任务号并排,同编号≠同任务）"; ws["A1"].font = TITLE
hdr = ["老系统任务号", "老系统任务名称", "newpm任务号", "newpm任务名称", "系统判断", "业务确认(请填)"]
ws.append([]); ws.append(hdr); style_header(ws, 3, len(hdr))
# [老任务号, 老任务名, 新任务号, 新任务名, 判断, 确认] —— 撞车行新旧任务号相同但任务名不同
task_rows = [
    ["M-202501-00030", "商户管理整合项目[第3期]", "M-202501-00030", "商户管理整合项目[第3期]", "✅名称一致 疑似同一", ""],
    ["M-202510-00065", "网络支付整合与能力提升项目[第55期]", "M-202510-00065", "TMS-【能力提升】-线下收单CRR086-273-201-中银智慧付静态二维码限额管理规则优化需求", "❌名称不同 疑似编号撞车", ""],
    ["M-202510-00632", "24年专项分期监管及审计整改项目[第10期]", "M-202510-00632", "24年专项分期监管及审计整改项目[第10期]", "✅名称一致 疑似同一", ""],
    ["M-202511-00610", "网络支付整合与能力提升项目[第63期]", "M-202511-00610", "TMS【能力提升】-线下收单CRR086-211-提升系统处理银联二维码反扫交易的成功率", "❌名称不同 疑似编号撞车", ""],
    ["M-202512-00694", "总对总汽车品牌购车线索管理及流程优化需求[第1期]", "M-202512-00694", "总对总汽车品牌购车线索管理及流程优化需求[第1期]", "✅名称一致 疑似同一", ""],
    ["M-202601-00522", "商户管理整合项目[第19期]", "M-202601-00522", "商户管理整合项目[第19期]", "✅名称一致 疑似同一", ""],
    ["M-202601-00645", "分行产品问题快速解决机制业务需求（中银智慧商家需求）[第4期]", "M-202601-00645", "分行产品问题快速解决机制业务需求（中银智慧商家需求）[第4期]", "✅名称一致 疑似同一", ""],
]
for row in task_rows:
    ws.append(row)
apply_borders(ws, 4, 3 + len(task_rows), len(hdr))
for i, row in enumerate(task_rows):
    is_clash = "撞车" in row[4]
    fill = BAD_FILL if is_clash else OK_FILL
    ws.cell(row=4 + i, column=5).fill = fill          # 系统判断列上色
    if is_clash:                                        # 撞车行: 新旧任务号两列也标红, 突出"号同名不同"
        ws.cell(row=4 + i, column=1).fill = BAD_FILL
        ws.cell(row=4 + i, column=3).fill = BAD_FILL
widths = [16, 40, 16, 50, 22, 20]
