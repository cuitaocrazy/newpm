...
rows = [
    ["", ""],
    ["核心结论", ""],
    ["newpm 生产现状", "52 个任务(33个有任务号, 2025-2026) / 19 个批次(2026)；5功能业务表基本为空"],
    ["老 yadapm 历史数据", "跨 2021-2026，约 7200 条业务记录，引用约 2900 任务 / 218 批次"],
    ["任务重叠", "老④引用202个去重任务，与生产仅重叠 7 个(其余195个生产没有)；其中2个为编号撞车"],
    ["批次重叠", "老218个批次，与生产仅重叠 5 个(其余213个生产没有)"],
...
q_rows = [
    ...
    [4, "占97%的『newpm没有的历史任务/批次』怎么呈现？ A反范式化(自带文本快照,推荐) / B注入老任务批次(污染日常任务管理) / C缩范围(先迁自包含功能)", ""],
    [5, "附件：老批次问题单仅47个附件、非批次0个，且物理文件不在备份(只有DB记录指向老服务器F:\\路径)。是否需单独找老服务器要附件文件？", ""],
]
for row in q_rows:
    ws.append(row)
