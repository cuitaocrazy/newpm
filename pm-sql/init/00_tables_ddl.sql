...
CREATE TABLE `pm_project` (
  ...
  `region_id` bigint DEFAULT NULL COMMENT '二级区域ID（关联pm_secondary_region表）',
  `short_name` varchar(50) DEFAULT NULL COMMENT '简称',
  `established_year` varchar(20) DEFAULT NULL COMMENT '立项年度',
  ...
  `region_code` varchar(50) DEFAULT NULL COMMENT '一级区域代码(字典:sys_yjqy)',
  ...
  `update_by` varchar(64) DEFAULT NULL COMMENT '更新者',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `revenue_confirm_status` varchar(20) DEFAULT '0' COMMENT '收入确认状态（字典:sys_qrzt）',
  `revenue_confirm_year` varchar(20) DEFAULT NULL COMMENT '收入确认年度',
  `revenue_confirm_date` date DEFAULT NULL COMMENT '收入确认日期',
  ...
  `company_revenue_confirmed_time` datetime DEFAULT NULL COMMENT '公司收入确认操作时间',
...
CREATE TABLE `pm_secondary_region` (
  `region_id` bigint NOT NULL AUTO_INCREMENT COMMENT '二级区域ID',
  `region_code` varchar(10) NOT NULL COMMENT '二级区域代码（行政区划代码前2位）',
  `region_name` varchar(50) NOT NULL COMMENT '二级区域名称',
  `region_type` varchar(20) DEFAULT '0' COMMENT '二级区域类型（0=省/1=直辖市/2=自治区/3=特别行政区/4=计划单列市）',
...
CREATE TABLE `pm_daily_report_detail` (
  ...
  `project_id` bigint DEFAULT NULL COMMENT '项目ID，entry_type=work时必填',
...
-- ===== feature 007 批次版本管理（出入库版本）=====
-- ============================================================
-- 批次版本管理（出入库版本）DDL — feature 007
-- 迁移自 yadapm T_B_VERSION_OUT / T_C_SYS_NAME
-- ============================================================

-- 1. 出入库版本主表（批次 + 非批次共用，manual_input 区分；本期仅批次=0）
...
-- 2. 版本-任务关联子表（最小化：只存关联键，显示字段 JOIN 取）
...
ALTER TABLE pm_version_out ADD COLUMN manual_task_no varchar(512) DEFAULT NULL COMMENT '手填软件中心任务号(非批次)/批次版本关联任务号快照';
...
CREATE TABLE IF NOT EXISTS pm_prolist_defect (
  ...
  task_no                   varchar(64)   DEFAULT NULL COMMENT '软件中心任务号(历史快照,无FK时用)',
  task_name                 varchar(255)  DEFAULT NULL COMMENT '任务名称(历史快照)',
  product                   varchar(64)   DEFAULT NULL COMMENT '二级产品(历史快照)',
  internal_closure_date     date          DEFAULT NULL COMMENT '提交内部测试B包日期(历史快照)',
  functional_test_date      date          DEFAULT NULL COMMENT '提交功能测试版本日期(历史快照)',
  production_version_date   date          DEFAULT NULL COMMENT '提交生产版本日期(历史快照)',
  schedule_status           varchar(32)   DEFAULT NULL COMMENT '排期状态(历史快照,sys_pqzt)',
  ...
  pro_batch_no              varchar(64)   DEFAULT NULL COMMENT '投产批次号(历史快照,无FK时用)',
  plan_production_date     date          DEFAULT NULL COMMENT '计划投产日期(历史快照,无batch_id时用)',
  dept_id                   bigint        DEFAULT NULL COMMENT '部门ID(项目组→部门,FK sys_dept)',
  subtask_team             varchar(64)   DEFAULT NULL COMMENT '项目组(历史快照,无dept_id时用)',
  remarks                   varchar(2048) DEFAULT NULL COMMENT '备注',
  del_flag                  char(1)       DEFAULT '0' COMMENT '删除标志(0正常1删除)',
  create_by                 varchar(64)   DEFAULT '' COMMENT '创建者',
  creator_name             varchar(64)   DEFAULT NULL COMMENT '创建人姓名(历史快照,老录入人)',
  create_time               datetime      DEFAULT NULL COMMENT '创建时间',
  update_by                 varchar(64)   DEFAULT '' COMMENT '更新者',
  modifier_name            varchar(64)   DEFAULT NULL COMMENT '修改人姓名(历史快照,老修改人)',
  update_time               datetime      DEFAULT NULL COMMENT '更新时间',
