...
| 对象 | 频率 | 本地热层 | OSS 前缀 | OSS 保留 | RPO |
|---|---|---|---|---|---|
| 数据库 `ry-vue` | 每 6 小时（00/06/12/18:10） | **留 2 份**（~10M） | `newpm-mysql/` | **30 天** | 6 小时 |
| 附件 upload-pvc | 每日 01:20 | **留 1 份**（~2.4G） | `newpm-upload/`（**单目录**） | **60 天** | 1 天 |

- **本地极小热层**：数据库留 2 份（消除新旧交替空窗），附件留 1 份（单份 2.4G 是省磁盘大头）。历史版本全靠 OSS。
