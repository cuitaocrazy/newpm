...
### 11.5 恢复演练结果（2026-07-27 实测通过）

对最新数据库备份 `newpm-mysql/newpm-20260727-1210.sql.gz`（5M）做了完整恢复演练：

| 步骤 | 结果 |
|---|---|
| 未解冻直接下载 | ✅ 报 `InvalidObjectState`（归档必须先解冻，符合预期） |
| 发起解冻 → 完成 | ✅ **21 秒**（`ongoing-request` true→false，解冻副本有效期 ~16 小时可反复下载） |
| 下载到 k3s001（内网） | ✅ 0.16 秒，免流量费 |
| `gzip -t` 完整性 | ✅ 未损坏，5110472 字节 |
| SQL 内容验证 | ✅ **58 张表齐全**，`Dump completed on` 结束标记在 |

**结论：备份可真实恢复，内容完好。RTO 起点约 21 秒（小文件/内网）。** 附件 2.4G 未演练，流程相同，解冻耗时会更长、取回费约 0.08 元。

> 演练需 `oss:RestoreObject` 权限，li.kong 最小权限策略默认无此权限。演练时临时全开 `oss:*`，**演练后已改回最小权限**（解冻低频操作建议改用主账号在控制台点，不动服务器 AK）。
