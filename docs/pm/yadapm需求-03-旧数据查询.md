...
| 旧数据查询 | pm_old_version_out | **只读** | 无 | 最简单，纯历史归档 ✅ 已实现(spec 009) |

---

## 九、落地结果（spec 009，2026-06-14 完成）

- **表**：新建 `pm_old_version_out`（19字段=id+18展示列，扁平快照，无 del_flag/审计）。
- **后端**：`OldVersionOutController` → `/project/oldVersionOut`，4 端点（`/list` + `/proBatchNoOptions` `/productOptions` `/versionTypeOptions`）全部 `@PreAuthorize('project:oldVersionOut:list')`，**无任何写接口**。Service 纯转发。
- **前端**：仅 `views/project/oldVersionOut/index.vue`（任务编号文本框 + 3个distinct下拉 + 18列表格 + 分页，**无操作列/无新增**）。
- **测试**：E2E 9/9 通过（4端点+各条件过滤+多条件组合+distinct去重）；浏览器UI真点验证（菜单/列表/下拉过滤/无错误）；后端全量回归 148/148 零回归；前端 build 退出码0。
- **Code Review**：零Critical零功能bug，修复迁移脚本幂等性（坑16）。
- **数据迁移**：`T_B_OLD_VERSION_OUT → pm_old_version_out` 属独立后续任务，功能已建空表上线，待旧库数据到位后导入。**注意**：核对源表 `version_descr/remarks` 等长文本字段长度，避免迁移截断（当前 varchar 512/2048）。
